...
class RocksdbWrapper {
 public:
  ...
  RocksdbWrapper(std::string path = default_db_path) : db_path(path) {
    ...
    if (!s.ok()) {
      ...
      insert_into_map(col_name_to_id_, db->DefaultColumnFamily()->GetName(),
                      db->DefaultColumnFamily()->GetID());
    } else {
      std::vector<ColumnFamilyDescriptor> column_families;
      for (auto column_name : column_families_list) {
        column_families.push_back(
            ColumnFamilyDescriptor(column_name, ColumnFamilyOptions()));
      }
  ...
  // Add ColumnFamily.
  int add_column(const char* name) { return do_add_column(name); }
  ...
  rocksdb::Status GetDbStatus() { return s; }
  ...
  // GET
  Slice Get(const Slice& key) { return Get(db->DefaultColumnFamily(), key); }
  ...
  iterator begin() const { return std::begin(handles); }
  iterator end() const { return std::end(handles); }
  size_t columns_count() const { return handles.size(); }
 ...
 private:
  ...
  void insert_into_map(std::unordered_map<std::string, colid_t> map,
                       std::string key, colid_t val) {
