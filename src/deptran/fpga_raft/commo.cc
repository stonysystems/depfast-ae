...
void FpgaRaftCommo::BroadcastPrepare(parid_t par_id,
                                       slotid_t slot_id,
                                       ballot_t ballot,
...
FpgaRaftCommo::BroadcastPrepare(parid_t par_id,
                                  slotid_t slot_id,
                                  ballot_t ballot) {
  int n = Config::GetConfig()->GetPartitionSize(par_id);
  auto e = Reactor::CreateSpEvent<FpgaRaftPrepareQuorumEvent>(n, n/2+1);
  auto proxies = rpc_par_proxies_[par_id];
  WAN_WAIT;
  for (auto& p : proxies) {
    auto proxy = (FpgaRaftProxy*) p.second;
...
FpgaRaftCommo::BroadcastAccept(parid_t par_id,
                                 slotid_t slot_id,
                                 ballot_t ballot,
                                 shared_ptr<Marshallable> cmd) {
  int n = Config::GetConfig()->GetPartitionSize(par_id);
//  auto e = Reactor::CreateSpEvent<FpgaRaftAcceptQuorumEvent>(n, n/2+1);
  auto e = Reactor::CreateSpEvent<FpgaRaftAcceptQuorumEvent>(n, n);
  auto proxies = rpc_par_proxies_[par_id];
  vector<Future*> fus;
  WAN_WAIT;
  for (auto& p : proxies) {
...
void FpgaRaftCommo::BroadcastAccept(parid_t par_id,
                                      slotid_t slot_id,
                                      ballot_t ballot,
                                      shared_ptr<Marshallable> cmd,
                                      const function<void(Future*)>& cb) {
