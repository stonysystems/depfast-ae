...
class FpgaRaftServer : public TxLogServer {
 private:
  ...
  void doVote(const slotid_t& lst_log_idx, const ballot_t& lst_log_term,
              const parid_t& can_id, const ballot_t& can_term,
              ballot_t* reply_term, bool_t* vote_granted, bool_t vote,
              const function<void()>& cb) {
    *vote_granted = vote;
    *reply_term = currentTerm;
    Log_debug(
        "loc %d vote decision %d, for can_id %d canterm %d curterm %d isleader "
        "%d lst_log_idx %d lst_log_term %d",
        loc_id_, vote, can_id, can_term, currentTerm, is_leader_, lst_log_idx,
        lst_log_term);
  ...
  int32_t randDuration() { return 4 + RandomGenerator::rand(0, 6); }
 ...
 public:
  ...
  void SetLocalAppend(shared_ptr<Marshallable>& cmd, uint64_t* term,
    ...
    *index = lastLogIndex;
    ...
    if (cmd->kind_ == MarshallDeputy::CMD_TPC_PREPARE) {
      ...
      // auto de = IO::write("/db", key_values, sizeof(struct KeyValue),
      // kv_vector.size());
  ...
  shared_ptr<FpgaRaftData> GetInstance(slotid_t id) {
    ...
    if (!sp_instance) sp_instance = std::make_shared<FpgaRaftData>();
    return sp_instance;
  }
  ...
  void OnVote(const slotid_t& lst_log_idx, const ballot_t& lst_log_term,
              const parid_t& can_id, const ballot_t& can_term,
              ballot_t* reply_term, bool_t* vote_granted,
              const function<void()>& cb);

  void OnVote2FPGA(const slotid_t& lst_log_idx, const ballot_t& lst_log_term,
                   const parid_t& can_id, const ballot_t& can_term,
                   ballot_t* reply_term, bool_t* vote_granted,
                   const function<void()>& cb);
  ...
  void OnForward(shared_ptr<Marshallable>& cmd, uint64_t* cmt_idx,
                 const function<void()>& cb);
