...
FpgaRaftServer::FpgaRaftServer(Frame * frame) {
  ...
  stop_ = false ;
  failover_ = Config::GetConfig()->get_failover();
  if (failover_) {
    timer_ = new Timer() ;
    resetTimer() ;
  }
...
void FpgaRaftServer::RequestVote() {
  ...
  if (sp_quorum->Yes()) {
    ...
    ((CoordinatorFpgaRaft*)co)->Submit(sp_m);   // TODO wait for reply 
...
void FpgaRaftServer::StartTimer()
{
  if(!init_ ){
    resetTimer() ;
    Coroutine::CreateRun([&]() {
        // TODO temp solution to avoid leadership changing if failover test option is off
        // to avoid some problems, for example open loop tps too high would cause unexpected election due to some jams in the coroutine,
        // better solution could be sending heartbeat from the leader to avoid leadership changing
        if (!failover_) return ;
