...
void FpgaRaftServer::Setup() {
  if (heartbeat_ && !FpgaRaftServer::looping && IsLeader()) {
    ...
    Pthread_create(&loop_th_, nullptr, FpgaRaftServer::HeartbeatLoop,
                   hb_loop_args);
  }
}
  ...
  while (FpgaRaftServer::looping) {
    ...
    for (auto it = matcheds.begin(); it != matcheds.end(); it++) {
      if (prevLogIndex > it->second + 10000 && cmd) {
        Log_info("leader_id: %d vs follower_id for %d: %d", prevLogIndex,
                 it->first, it->second);
        // hb_loop_args->commo->SendHeartbeat(partition_id, it->first,
        // prevLogIndex);
        hb_loop_args->commo->SendAppendEntriesAgain(
            it->first, partition_id, slot, ballot,
            hb_loop_args->sch->IsLeader(), term, prevLogIndex, prevTerm,
            hb_loop_args->sch->commitIndex, cmd);
      }
    }
  }
  delete hb_loop_args;
...
void FpgaRaftServer::RequestVote2FPGA() {
  ...
  if (paused_) {
    resetTimer();
    Log_debug("fpga raft server %d request vote to fpga rejected due to paused",
              loc_id);
    // req_voting_ = false ;
    return;
  }
  ...
  auto sp_quorum =
      ((FpgaRaftCommo *)(this->commo_))
          ->BroadcastVote2FPGA(par_id, lst_idx, lst_term, loc_id, currentTerm);
  sp_quorum->Wait();
...
void FpgaRaftServer::OnVote2FPGA(const slotid_t &lst_log_idx,
                                 ...
                                 const ballot_t &can_term, ballot_t *reply_term,
                                 bool_t *vote_granted,
  ...
  if (can_term < cur_term) {
    doVote(lst_log_idx, lst_log_term, can_id, can_term, reply_term,
           vote_granted, false, cb);
    return;
  }
  ...
  Log_debug("vote for lstoff %d, curlstterm %d, curlstidx %d", lstoff,
            curlstterm, curlstidx);

  // TODO del only for test
  verify(lstoff == lastLogIndex);

  if (lst_log_term > curlstterm ||
      (lst_log_term == curlstterm && lst_log_idx >= curlstidx)) {
    doVote(lst_log_idx, lst_log_term, can_id, can_term, reply_term,
           vote_granted, true, cb);
    return;
  }

  doVote(lst_log_idx, lst_log_term, can_id, can_term, reply_term, vote_granted,
         false, cb);
}
...
bool FpgaRaftServer::RequestVote() {
  ...
  if (paused_) {
    Log_debug("fpga raft server %d request vote rejected due to paused",
              loc_id);
    resetTimer();
    // req_voting_ = false ;
    return false;
  ...
  auto sp_quorum =
      ((FpgaRaftCommo *)(this->commo_))
          ->BroadcastVote(par_id, lst_idx, lst_term, loc_id, currentTerm);
  sp_quorum->Wait();
  std::lock_guard<std::recursive_mutex> lock1(mtx_);
  if (sp_quorum->Yes()) {
    // become a leader
    ...
    if (IsLeader()) {
      ...
      Log_debug("fpga vote rejected %d curterm %d, do rollback", loc_id,
                currentTerm);
      setIsLeader(false);
      return false;
    }
...
void FpgaRaftServer::OnVote(const slotid_t &lst_log_idx,
                            const ballot_t &lst_log_term, const parid_t &can_id,
                            const ballot_t &can_term, ballot_t *reply_term,
                            bool_t *vote_granted, const function<void()> &cb) {
  std::lock_guard<std::recursive_mutex> lock(mtx_);
  Log_debug("fpga raft receives vote from candidate: %llx", can_id);
...
void FpgaRaftServer::OnAppendEntries(
    const slotid_t slot_id, const ballot_t ballot,
    const uint64_t leaderCurrentTerm, const uint64_t leaderPrevLogIndex,
    const uint64_t leaderPrevLogTerm, const uint64_t leaderCommitIndex,
    const struct DepId dep_id, shared_ptr<Marshallable> &cmd,
    uint64_t *followerAppendOK, uint64_t *followerCurrentTerm,
    uint64_t *followerLastLogIndex, const function<void()> &cb) {
  std::lock_guard<std::recursive_mutex> lock(mtx_);
  // StartTimer() ;

  Log_debug(
      "fpga-raft scheduler on append entries for "
  ...
  if ((leaderCurrentTerm >= this->currentTerm) &&
    ...
    /*if (slot_id == 100000000 || leaderPrevLogIndex + 1 < lastLogIndex) {
            for (int i = 0; i < 1000000; i++) Log_info("Dropping this AE
    message: %d %d", leaderPrevLogIndex, lastLogIndex);
            //verify(0);
            *followerAppendOK = 0;
            cb();
            return;
    ...
    *followerLastLogIndex = this->lastLogIndex;
    ...
    if (cmd->kind_ == MarshallDeputy::CMD_TPC_PREPARE) {
      ...
      auto sp_vec_piece =
          dynamic_pointer_cast<VecPieceData>(p_cmd->cmd_)->sp_vec_piece_data_;
      ...
      // auto de = IO::write("/db/data.txt", key_values, sizeof(struct
      // KeyValue), kv_vector.size());
      ...
      auto sp_func = [&key_values, kv_sz, lastlogindex]() {
        ...
        auto val =
            rdb::RocksdbWrapper::MakeSlice(reinterpret_cast<char *>(key_values),
                                           sizeof(struct KeyValue) * kv_sz);
      ...
      auto sp_func = [&value, lastlogindex]() {
        std::string index_key = std::to_string(lastlogindex);
        auto val = rdb::RocksdbWrapper::MakeSlice(
            reinterpret_cast<char *>(&value), sizeof(int));
        rdb::rocksdb_wrapper()->Put(index_key, val);
      };
      auto de = IO::write(sp_func);
      de->Wait();
    ...
    Log_debug(
        "reject append loc: %d, leader term %d last idx %d, server term: %d "
        "last idx: %d",
        this->loc_id_, leaderCurrentTerm, leaderPrevLogIndex, currentTerm,
        lastLogIndex);
    *followerAppendOK = 0;
  }
...
void FpgaRaftServer::OnForward(shared_ptr<Marshallable> &cmd, uint64_t *cmt_idx,
                               const function<void()> &cb) {
  this->rep_frame_ = this->frame_;
  auto co = ((TxLogServer *)(this))->CreateRepCoord(0);
...
void FpgaRaftServer::OnCommit(const slotid_t slot_id, const ballot_t ballot,
  ...
  for (slotid_t id = executeIndex + 1; id <= commitIndex; id++) {
    ...
    if (next_instance->log_) {
      Log_debug("fpga-raft par:%d loc:%d executed slot %lx now", partition_id_,
                loc_id_, id);
  ...
  Log_info("time of decide on server: %d", (end.tv_sec -
  begin.tv_sec)*1000000000 + end.tv_nsec - begin.tv_nsec);*/
}
void FpgaRaftServer::SpCommit(const uint64_t cmt_idx) {
  verify(0);  // TODO delete it
  std::lock_guard<std::recursive_mutex> lock(mtx_);
  Log_debug("fpga raft spcommit for index: %lx for server %d", cmt_idx,
            loc_id_);
  verify(cmt_idx != 0);
