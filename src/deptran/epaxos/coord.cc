...
//  verify(tx_data().partition_ids_.size() == dtxn->partition_.size());
  vector<shared_ptr<EPaxosPreAcceptQuorumEvent>> events;
...
int32_t EPaxosCoord::GetFastQuorum(parid_t par_id) {
  int32_t n = Config::GetConfig()->GetPartitionSize(par_id);
  return n;
}

int32_t EPaxosCoord::GetSlowQuorum(parid_t par_id) {
  int32_t n = Config::GetConfig()->GetPartitionSize(par_id);
  return n / 2 + 1;
}
 ...
 */
bool EPaxosCoord::FastQuorumGraphCheck(EPaxosPreAcceptQuorumEvent& ev) {
