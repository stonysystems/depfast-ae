            ...
            if (progress >= 5 and self.once == 1):
                try:
                    ...
                    cmd = "pid=`ss -tulpn | grep '0.0.0.0:10000' | awk '{print $7}' | cut -f2 -d= | cut -f1 -d,`; \
                           ...
                           echo '8:32 64000' | sudo tee /sys/fs/cgroup/blkio/janus/blkio.throttle.read_iops_device; \
                           echo '8:32 64000' | sudo tee /sys/fs/cgroup/blkio/janus/blkio.throttle.write_iops_device; \
