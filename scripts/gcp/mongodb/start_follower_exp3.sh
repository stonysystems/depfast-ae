...
echo "Running follower experiment 3 for mongodb"
...
do
	...
	# 4. Run experiment
	# Slow down follower
	./experiment3.sh "$secondaryip" "$secondarypid"

	# 5. ycsb run
	./bin/ycsb run mongodb -s -P workloads/workloada_more  -p maxexecutiontime=900 -p mongodb.url=mongodb://$primaryip:27017/ycsb?w=majority&readConcernLevel=majority > results/exp3_trial_$i.txt
