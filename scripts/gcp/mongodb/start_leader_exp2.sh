...
echo "Running leader experiment 2 for mongodb"
...
do
    echo "Running experiment 2 - Trial $i"
	# 0. Cleanup first
	...
	# 5. Run experiment
	# Slow down leader
	./experiment2.sh "$primaryip" "$primarypid"

	# 5. ycsb run
	./bin/ycsb run mongodb -s -P workloads/workloada_more  -p maxexecutiontime=900 -p mongodb.url=mongodb://$primaryip:27017/ycsb?w=majority&readConcernLevel=majority > leader_results/exp2_trial_$i.txt

	# 6. cleanup
	mongo --host "$primaryip" < cleanup_script.js
	mongo --host "$primaryip" --eval "db.getCollectionNames().forEach(function(n){db[n].remove()});"
	ssh -i ~/.ssh/id_rsa "$s1" "sudo sh -c 'rm -rf /data/db ; sudo umount /dev/sdb ; rm -rf /data/ ; sudo cgdelete cpu:cpulow cpu:cpuhigh  ; pkill mongo ; true'"
	ssh -i ~/.ssh/id_rsa "$s2" "sudo sh -c 'rm -rf /data/db ; sudo umount /dev/sdb ; rm -rf /data/ ; sudo cgdelete cpu:cpulow cpu:cpuhigh ; pkill mongo ; true'"
	ssh -i ~/.ssh/id_rsa "$s3" "sudo sh -c 'rm -rf /data/db ; sudo umount /dev/sdb ; rm -rf /data/ ; sudo cgdelete cpu:cpulow cpu:cpuhigh ; pkill mongo ; true'"
