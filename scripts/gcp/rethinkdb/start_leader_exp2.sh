...
for i in {1..5}
do
    echo "Running Leader experiment 2 - Trial $i"
	# 0. Cleanup first
	...
	# 5. Run experiment 2
	ssh -i ~/.ssh/id_rsa "$primaryip" "sh -c 'nohup taskset -ac 0 ~/deadloop > /dev/null 2>&1 &'"
	deadlooppid=$(ssh -i ~/.ssh/id_rsa "$primaryip" "sh -c 'pgrep deadloop'")
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo mkdir /sys/fs/cgroup/cpu/cpulow /sys/fs/cgroup/cpu/cpuhigh'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo 64 > /sys/fs/cgroup/cpu/cpulow/cpu.shares'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $deadlooppid > /sys/fs/cgroup/cpu/cpuhigh/cgroup.procs'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $primarypid > /sys/fs/cgroup/cpu/cpulow/cgroup.procs'"

	# 6. ycsb run
	./bin/ycsb run rethinkdb -s -P workloads/workloada_more -p maxexecutiontime=900 -p rethinkdb.host=$primaryip -p rethinkdb.port=28015 >> leader_results/exp2_trial_$i.txt
