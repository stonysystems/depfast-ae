...
for i in {1..5}
do
    echo "Running follower experiment 2 - Trial $i"
	# 0. Cleanup first
	...
	# 6. ycsb run
	./bin/ycsb run rethinkdb -s -P workloads/workloada_more -p maxexecutiontime=900 -p rethinkdb.host=$primaryip -p rethinkdb.port=28015 >> results/exp2_trial_$i.txt
	...
	ssh -i ~/.ssh/id_rsa "$s2" "sh -c 'rm -rf /data1/*'"
	ssh -i ~/.ssh/id_rsa "$s3" "sh -c 'rm -rf /data1/*'"
	ssh -i ~/.ssh/id_rsa "$secondaryip" "sudo sh -c 'pgrep deadloop | xargs -n1 kill | true'"
	#ssh -i ~/.ssh/id_rsa "$secondaryip" "sudo sh -c 'sudo cgdelete cpu:cpuhigh cpu:cpulow'"
