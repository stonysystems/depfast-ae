...
for i in {1..5}
do
    echo "Running leader experiment 3 - Trial $i"
	# 0. Cleanup first
	...
	# 5. Run experiment 2
	# 
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo mkdir /sys/fs/cgroup/blkio/cockroachdb'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sync; echo 3 > /proc/sys/vm/drop_caches'"
	lsblkcmd="8:16 524288"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $lsblkcmd > /sys/fs/cgroup/blkio/cockroachdb/blkio.throttle.read_bps_device'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $lsblkcmd > /sys/fs/cgroup/blkio/cockroachdb/blkio.throttle.write_bps_device'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $primarypid > /sys/fs/cgroup/blkio/cockroachdb/cgroup.procs'"
	
	# 6. ycsb run
	./bin/ycsb run rethinkdb -s -P workloads/workloada_more -p maxexecutiontime=900 -p rethinkdb.host=$primaryip -p rethinkdb.port=28015 >> leader_results/exp3_trial_$i.txt
