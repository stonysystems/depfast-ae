...
do
	...
	# 5. Run experiment
	# Slow down leader
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo mkdir /sys/fs/cgroup/cpu/cockroachdb'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo 50000 > /sys/fs/cgroup/cpu/cockroachdb/cpu.cfs_quota_us'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo 1000000 > /sys/fs/cgroup/cpu/cockroachdb/cpu.cfs_period_us'"
	ssh -i ~/.ssh/id_rsa "$primaryip" "sudo sh -c 'sudo echo $primarypid > /sys/fs/cgroup/cpu/cockroachdb/cgroup.procs'"

	# 6. ycsb run
	./bin/ycsb run rethinkdb -s -P workloads/workloada_more -p maxexecutiontime=900 -p rethinkdb.host=$primaryip -p rethinkdb.port=28015 >> leader_results/exp1_trial_$i.txt
