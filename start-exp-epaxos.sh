...
if [[ $5 == "3" ]]; then
	...
	./run_all$exp.py -d $3 -hh config/hosts-nonlocal-5.yml -s '1:2:1' -c $nc:$((nc+1)):1 -r '5' -cc config/tpca.yml -cc config/client_closed.yml -cc config/none_copilot.yml -cc config/concurrent_$2.yml -b tpca -m $cc:copilot $1
fi
