...
if [[ $5 == "3" ]]; then
	./run_all$exp.py -d $3 -hh config/hosts-nonlocal.yml -s '1:2:1' -c '1:2:1' -r '3' -cc config/tpca.yml -cc config/client_closed.yml -cc config/none_copilot.yml -cc config/concurrent_$2.yml -b tpca -m $cc:copilot $1
else
...
cd ../
echo $(pwd)
tar xzf depfast/archive/$1-tpca_${cc}-copilot_1_1_-1.tgz
log=log/$1-tpca_$cc-copilot_1_1_-1.log
